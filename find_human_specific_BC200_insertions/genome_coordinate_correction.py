...
for record in SeqIO.parse("ins_sequence_with_BC200gene_aligned.fasta", "fasta"):
 ...
 if(title=='BC200#BC200_gene_consensus_sequence'):
  ...
  if(strand=='C'):
   ...
   new_seq=reverse_complement(str(fasta[chrno][l_site:r_site-duoyu]))
   
   new_title='>'+chrno+'*'+str(l_site+1)+'*'+str(r_site-duoyu)+'*'+'C'
   ...
   new_seq=str(fasta[chrno][l_site+duoyu:r_site])
   
   new_title='>'+chrno+'*'+str(l_site+duoyu+1)+'*'+str(r_site)+'*'+'+'
